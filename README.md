...
```mermaid
flowchart TB
    subgraph TARGET["🖥️ Target Machine (Monitored Endpoint)"]
        CSV["Auth Logs · CSV\nGenerate-EnrichedAuthLogs.ps1"]
        JSON["Firewall Logs · JSON\nGenerate-FirewallLogs-JSON.ps1"]
        AGENT["System Monitor Agent\nFIM: C:\\FIM_TEST\\"]
        FIMSCRIPT["FIM Activity Script\nGenerate-FIM-Activity.ps1"]
    end

    subgraph LR["⚙️ LogRhythm Platform (LR-XMVS_MACHINE)"]
        DP["Data Processor\nMPE · CEF Normalization"]
        PM["Platform Manager\nRBAC · Web Console"]
        AIE["AI Engine\nCorrelation · Detection"]
        DI["Data Indexer\nStorage · Search"]

        subgraph SOURCES["Custom Log Source Types"]
            S1["CSV Log Source\nAuth events · field mapping"]
            S2["JSON Log Source\nFirewall events · field mapping"]
        end

        subgraph ALARMS["AI Engine Alarms"]
            A1["🔴 Brute Force — T1110\n≥5 failed logins / 10 min"]
            A2["🟠 After-Hours Access — T1005\ntime-based correlation filter"]
        end

        subgraph FIM["FIM Alarms — T1565"]
            F1["📁 File Added"]
            F2["✏️ File Modified"]
            F3["🗑️ File Deleted"]
        end

        GRAFANA["📊 Grafana — Platform Health Observability\nEPS · Component Status · Alarm Rate"]
    end

    CSV  -->|log ingestion| DP
    JSON -->|log ingestion| DP
    AGENT -->|FIM events| AIE
    FIMSCRIPT -->|simulates changes| AGENT
